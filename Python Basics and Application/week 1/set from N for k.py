...
n_elem, k_elem = (int(i) for i in input().split())  # генератор кортежа, списка


# Второй способ записи, через функцию map
# n, k = map(int, input().split())


def set_from_n_for_k(n, k):
    if k == 0:
        return 1
    if k > n:
        return 0
    return set_from_n_for_k(n - 1, k) + set_from_n_for_k(n - 1, k - 1)


print(set_from_n_for_k(n_elem, k_elem))
